/*
Question :
Given a string S with repeated characters (only lowercase). The task is to rearrange characters in a string such that no two adjacent characters are same. It may 
be assumed that the string has only lowercase English alphabets. Complete the function which takes the argument as a string of lowercase English alphabets and 
prints 1 if the generated string doesn't contain any same adjacent characters, else if no such string is possible to be made print 0
...
2. Create a temporary Key that will be used as the previously visited element (the previous element in the resultant string. Initialize it { char = ‘#’ , freq = 
‘-1’ } 
...
Video Link : https://drive.google.com/file/d/1us_9wmqjSTZblOQAvw4pXiawP4i3jynN/view?usp=sharing
